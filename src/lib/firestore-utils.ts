...
const isConnectionError = (error: any): boolean => {
  ...
  if (errorStr.includes('permission-denied') || errorStr.includes('insufficient permissions')) {
    return false;
  }
...
export const safeUpdateDoc = async (
  ...
  try {
    ...
    console.error(`Firestore updateDoc error (attempt ${retryCount + 1}):`, error);
...
export const safeGetDocs = async (
  ...
  try {
    ...
    console.error(`Firestore getDocs error (attempt ${retryCount + 1}):`, error);
